...
const dbName = 'search-v2';
const dbStoreDocs = 'docs';
const dbStoreIndex = 'index';
const dbIndexQuery = 'query';
const dbRO = 'readonly';
...
function dbOpen(name) {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = event => {
      ...
      let indexStore = db.createObjectStore(dbStoreIndex, {autoIncrement: true});
      db.createObjectStore(dbStoreDocs, {autoIncrement: true});
      indexStore.createIndex(dbIndexQuery, 'term', { unique: false });
    };
  });
}
    ...
    return new Promise((resolve, reject) => {
      ...
      for (let i = 0; i <= terms.length; i++) {
        if (i === 0 || terms[i] === terms[i - 1]) {
          ...
          store.put({term: terms[i - 1], id: docId, tf: wordCount / terms.length});
          wordCount = 1;
        }
      }
    })
